...
class Job_Search_Public {
	 ...
	 *
	 * @since 		1.0.0
	 * @param 		string 			$Job_Search 		The name of the plugin.
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/job-search-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/job-search-public.js', array( 'jquery' ), $this->version, true );
	...
	public function how_to_apply() {
		...
		include Job_Search_get_template( 'job-search-how-to-apply' );
	...
	public function list_openings( $atts = array() ) {
		...
		$shared 					= new Job_Search_Shared( $this->plugin_name, $this->version );
		$items 						= $shared->get_openings( $args );

		if ( is_array( $items ) || is_object( $items ) ) {

			include Job_Search_get_template( $args['loop-template'] );
	...
	public function single_cpt_template( $template ) {
	    ...
	    if ( $post->post_type == 'job' ) {
			...
			$return = Job_Search_get_template( 'single-job' );
