...
class Job_Search_Admin {
	 ...
	 *
	 * @since 		1.0.0
	 * @param 		string 			$Job_Search 		The name of this plugin.
    ...
    public static function add_admin_notices() {
    	...
    	$notices 	= get_option( 'Job_Search_deferred_admin_notices', array() );
  		//$notices[] 	= array( 'class' => 'updated', 'notice' => esc_html__( 'Now Hiring: Custom Activation Message', 'job-search' ) );
  		//$notices[] 	= array( 'class' => 'error', 'notice' => esc_html__( 'Now Hiring: Problem Activation Message', 'job-search' ) );

  		apply_filters( 'Job_Search_admin_notices', $notices );
  		update_option( 'Job_Search_deferred_admin_notices', $notices );
	...
	public function add_menu() {
		...
		add_submenu_page(
			...
			apply_filters( $this->plugin_name . '-settings-page-title', esc_html__( 'Now Hiring Settings', 'job-search' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', esc_html__( 'Settings', 'job-search' ) ),
			'manage_options',
		...
		add_submenu_page(
			'edit.php?post_type=job',
			apply_filters( $this->plugin_name . '-settings-page-title', esc_html__( 'Now Hiring Help', 'job-search' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', esc_html__( 'Help', 'job-search' ) ),
			'manage_options',
	...
	public function display_admin_notices() {
		...
		$notices = get_option( 'Job_Search_deferred_admin_notices' );
		...
		delete_option( 'Job_Search_deferred_admin_notices' );
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/job-search-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		if ( 'job' === $post_type || $screen->id === $hook_suffix ) {
			...
			$localize['repeatertitle'] = __( 'File Name', 'job-search' );

			wp_localize_script( 'job-search', 'nhdata', $localize );
...
*/
	...
	public function link_settings( $links ) {
		...
		$links[] = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'edit.php?post_type=job&page=' . $this->plugin_name . '-settings' ) ), esc_html__( 'Settings', 'job-search' ) );
	...
	public static function new_cpt_job() {
		...
		$opts['labels']['add_new']						= esc_html__( "Add New {$single}", 'job-search' );
		$opts['labels']['add_new_item']					= esc_html__( "Add New {$single}", 'job-search' );
		$opts['labels']['all_items']					= esc_html__( $plural, 'job-search' );
		$opts['labels']['edit_item']					= esc_html__( "Edit {$single}" , 'job-search' );
		$opts['labels']['menu_name']					= esc_html__( $plural, 'job-search' );
		$opts['labels']['name']							= esc_html__( $plural, 'job-search' );
		$opts['labels']['name_admin_bar']				= esc_html__( $single, 'job-search' );
		$opts['labels']['new_item']						= esc_html__( "New {$single}", 'job-search' );
		$opts['labels']['not_found']					= esc_html__( "No {$plural} Found", 'job-search' );
		$opts['labels']['not_found_in_trash']			= esc_html__( "No {$plural} Found in Trash", 'job-search' );
		$opts['labels']['parent_item_colon']			= esc_html__( "Parent {$plural} :", 'job-search' );
		$opts['labels']['search_items']					= esc_html__( "Search {$plural}", 'job-search' );
		$opts['labels']['singular_name']				= esc_html__( $single, 'job-search' );
		$opts['labels']['view_item']					= esc_html__( "View {$single}", 'job-search' );

		$opts['rewrite']['ep_mask']						= EP_PERMALINK;
		$opts['rewrite']['feeds']						= FALSE;
		$opts['rewrite']['pages']						= TRUE;
		$opts['rewrite']['slug']						= esc_html__( strtolower( $plural ), 'job-search' );
		$opts['rewrite']['with_front']					= FALSE;

		$opts = apply_filters( 'job-search-cpt-options', $opts );
	...
	public static function new_taxonomy_type() {
		...
		$opts['labels']['add_new_item'] 				= esc_html__( "Add New {$single}", 'job-search' );
		$opts['labels']['add_or_remove_items'] 			= esc_html__( "Add or remove {$plural}", 'job-search' );
		$opts['labels']['all_items'] 					= esc_html__( $plural, 'job-search' );
		$opts['labels']['choose_from_most_used'] 		= esc_html__( "Choose from most used {$plural}", 'job-search' );
		$opts['labels']['edit_item'] 					= esc_html__( "Edit {$single}" , 'job-search');
		$opts['labels']['menu_name'] 					= esc_html__( $plural, 'job-search' );
		$opts['labels']['name'] 						= esc_html__( $plural, 'job-search' );
		$opts['labels']['new_item_name'] 				= esc_html__( "New {$single} Name", 'job-search' );
		$opts['labels']['not_found'] 					= esc_html__( "No {$plural} Found", 'job-search' );
		$opts['labels']['parent_item'] 					= esc_html__( "Parent {$single}", 'job-search' );
		$opts['labels']['parent_item_colon'] 			= esc_html__( "Parent {$single}:", 'job-search' );
		$opts['labels']['popular_items'] 				= esc_html__( "Popular {$plural}", 'job-search' );
		$opts['labels']['search_items'] 				= esc_html__( "Search {$plural}", 'job-search' );
		$opts['labels']['separate_items_with_commas'] 	= esc_html__( "Separate {$plural} with commas", 'job-search' );
		$opts['labels']['singular_name'] 				= esc_html__( $single, 'job-search' );
		$opts['labels']['update_item'] 					= esc_html__( "Update {$single}", 'job-search' );
		$opts['labels']['view_item'] 					= esc_html__( "View {$single}", 'job-search' );

		$opts['rewrite']['ep_mask']						= EP_NONE;
		$opts['rewrite']['hierarchical']				= FALSE;
		$opts['rewrite']['slug']						= esc_html__( strtolower( $tax_name ), 'job-search' );
		$opts['rewrite']['with_front']					= FALSE;

		$opts = apply_filters( 'job-search-taxonomy-options', $opts );
	...
	public function page_help() {
		...
		include( plugin_dir_path( __FILE__ ) . 'partials/job-search-admin-page-help.php' );
	...
	public function register_fields() {
		...
		add_settings_field(
			'message-no-openings',
			apply_filters( $this->plugin_name . 'label-message-no-openings', esc_html__( 'No Openings Message', 'job-search' ) ),
		...
		add_settings_field(
			'how-to-apply',
			apply_filters( $this->plugin_name . 'label-how-to-apply', esc_html__( 'How to Apply', 'job-search' ) ),
		...
		add_settings_field(
			'repeater-test',
			apply_filters( $this->plugin_name . 'label-repeater-test', esc_html__( 'Repeater Test', 'job-search' ) ),
	...
	public function register_sections() {
		...
		add_settings_section(
			$this->plugin_name . '-messages',
			apply_filters( $this->plugin_name . 'section-title-messages', esc_html__( 'Messages', 'job-search' ) ),
	...
	private function sanitizer( $type, $data ) {
		...
		$return 	= '';
		$sanitizer 	= new Job_Search_Sanitize();
	...
	public function section_messages( $params ) {
		...
		include( plugin_dir_path( __FILE__ ) . 'partials/job-search-admin-section-messages.php' );
	...
	public function validate_options( $input ) {
		...
		foreach ( $options as $option ) {
			...
			if ( 'repeater' === $type && is_array( $option[2] ) ) {
				...
				$count = Job_Search_get_max( $clean );
			...
			if ( $valid[$option[0]] != $input[$option[0]] ) {
				...
				add_settings_error( $option[0], $option[0] . '_error', esc_html__( $option[0] . ' error.', 'job-search' ), 'error' );
